...
class Collector:
    ...
    def verbose_report(self, verdict: bool, type: str, summary: Summary, schema: Optional[SchemaResponse]):
        ...
        if schema:
            fields = len(
                schema.fieldDefinitions) if schema.fieldDefinitions else 0
        ...
        if click:
            click.secho(icon, nl=False,
                        bg='green' if verdict else 'red', fg='white')
    ...
    async def collect(self) -> list[str]:
        ...
        for bo_type in ['Major', 'Supporting', 'Lookup', 'Groups']:
            for bo_summary in await self._instance.get_bo_summaries(type=cast(BusinessObjectType, bo_type)):
